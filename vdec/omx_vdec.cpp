...
//    m2_cedarv_req_ctx.usage = CEDARV_USAGE_VIDEO;
//	if (RequestCedarVResource(&m2_cedarv_req_ctx) < 0) {
//		ALOGE("cedarv_req_ctx init fail!!");
//		eRet = OMX_ErrorInsufficientResources;
//        goto EXIT;
//	}
                                      ...
                                      OMX_IN OMX_PTR         cmdData)
{
    ...
    switch (cmd)
    {
        ...
        case OMX_CommandPortEnable:
            ...
            //ACodec.cpp, ACodec::OutputPortSettingsChangedState::onOMXEvent(), case OMX_EventCmdComplete,
            //CHECK_EQ(mCodec->mOMX->sendCommand(
            //                mCodec->mNode, OMX_CommandPortEnable, kPortIndexOutput),
            //             (status_t)OK);
            // need take effect immediately!
            if (param1 == 0x0 || (OMX_S32)param1 == -1)
            {
                ALOGD("InPortEnable immediately");
                m_sInPortDef.bEnabled = OMX_TRUE;
            }

            if (param1 == 0x1 || (OMX_S32)param1 == -1)
            {
                ALOGD("OutPortEnable immediately");
                m_sOutPortDef.bEnabled = OMX_TRUE;
            }
	        eCmd = RestartPort;
	        break;
    ...
    switch(paramIndex)
    {
    	...
    	case OMX_IndexParamPortDefinition:
    	{
	        ...
	        else if (((OMX_PARAM_PORTDEFINITIONTYPE*)(paramData))->nPortIndex == m_sOutPortDef.nPortIndex)
	        {
                ...
                ALOGV(" get_OMX_IndexParamPortDefinition: m_sOutPortDef.nPortIndex[%d]", (int)m_sOutPortDef.nPortIndex);
	            ALOGV("x width = %d, height = %d", (int)m_sOutPortDef.format.video.nFrameWidth, (int)m_sOutPortDef.format.video.nFrameHeight);
	        }
	        else
	        {
        							...
        							OMX_IN  OMX_BUFFERHEADERTYPE* pBufferHdr)
{
    ...
    ALOGV("(f:%s, l:%d) nPortIndex = %d, pBufferHdr = %p", __FUNCTION__, __LINE__, (int)nPortIndex, pBufferHdr);
...
//                            		pSelf->m_decoder->ioctrl(pSelf->m_decoder, CEDARV_COMMAND_SET_MAX_OUTPUT_HEIGHT, 1288); //2304, 1288
        	...
        	port_setting_match)
        {
        	...
        	if(pSelf->m_decoder->picture_ready(pSelf->m_decoder))
        	{
            	...
            	{
            		usleep(20*1000);
                    //ALOGV("no output frame,wait 20ms");
            		continue;
            	...
            	pOutBufHdr->nFilledLen = picture.width*picture.height*3/2;
                ...
                //ALOGV("FillBufferDone is called, pts[%lld]", pOutBufHdr->nTimeStamp);
        	...
        	else
        	{
        		while(1)
        		{
        			...
        			if(decodeResult == CEDARV_RESULT_FRAME_DECODED ||
        			   ...
        			   decodeResult == CEDARV_RESULT_NO_FRAME_BUFFER)
        			{
        				break;
        			}
